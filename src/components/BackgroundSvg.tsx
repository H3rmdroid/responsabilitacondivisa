...
export function BackgroundSvg({ storieCambiamentoRef }: BackgroundSvgProps) {
  ...
  // Opacità globale dell'SVG che va da 0 a 1 (più veloce)
  ...
  useEffect(() => {
    console.log("📏 Background section progress (init):", (lateralProgress.get() * 100).toFixed(1), "%");
    console.log("🧵 Lateral Path Length (init):", lateralPathLength.get().toFixed(3));
  }, [lateralProgress, lateralPathLength]);
  useMotionValueEvent(lateralProgress, "change", (latest) => {
    console.log("📏 Background section progress:", (latest * 100).toFixed(1), "%");
  });
  useMotionValueEvent(lateralPathLength, "change", (latest) => {
    console.log("🧵 Lateral Path Length Value:", latest.toFixed(3));
  });
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* PATH DIETRO AL TITOLO - Ordinati dall'alto verso il basso (Y più piccolo = più in alto) */}
          <motion.path
            className="cls-1"
